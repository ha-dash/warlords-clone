...
export class Map {
     ...
     */
    getNeighbors(x, y) {
        const coords = this.getNeighborCoordinates(x, y);
        return coords.map(c => this.getHex(c.x, c.y)).filter(h => h !== null);
    }
     ...
     */
    calculateMovementCost(fromX, fromY, toX, toY, unit = null, options = {}) {
        ...
        if (!options.ignorePassable && !toHex.isPassable(unit)) {
            return Infinity;
        }
